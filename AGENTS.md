...
## Project Overview
This is a research project for neutron/X-ray reflectometry analysis using the **Reflectorch** machine learning package. The project focuses on analyzing experimental reflectivity data to determine scattering length density (SLD) profiles and structural parameters of thin films.
...
Agents in this environment are designed for:
- **Code Review**: Analyzing code quality, identifying improvements, and ensuring consistency
- **Documentation**: Creating and maintaining technical documentation
- **Code Writing**: Implementing features, fixes, and enhancements

### Domain Knowledge Requirements
Agents should understand core neutron reflectometry concepts:
- **Neutron Reflectometry**: Technique for analyzing thin film structures using neutron scattering
- **Scattering Length Density (SLD)**: Key physical parameter describing material composition
- **Q-space Analysis**: Momentum transfer analysis in reciprocal space
- **Reflectivity Curves**: Experimental data relating incident angle to reflected intensity

### Version Control Awareness
- **Git Integration**: Always be aware of current repository state and pending changes
- **Commit Messages**: Write clear, descriptive commit messages following project conventions
- **Change Tracking**: Monitor file modifications and their impact on the codebase
- **MCP Servers**: Utilize installed Model Context Protocol servers for enhanced capabilities

### Output & Communication Standards
- **Brevity**: Keep responses brief and concise - avoid verbose explanations
- **Precision**: Focus on essential information without unnecessary elaboration
- **Technical Accuracy**: Ensure all technical details are correct and domain-appropriate

### Code Quality & Safety Guidelines

#### Critical Change Review
- **Comment Requirement**: Always comment on critical changes that affect:
...
- **Duplicate Detection**: Actively look for existing code that performs similar functions
- **Reuse Priority**: Always prefer reusing existing utilities over creating new implementations
- **Modular Approach**: Identify opportunities to extract reusable components

#### Implementation Philosophy
- **No Placeholders**: Never implement fallback values, default behaviors, or placeholder code unless explicitly requested
- **Explicit Requirements**: Only implement what is specifically asked for
- **Minimal Scope**: Avoid over-engineering solutions or adding "nice-to-have" features

#### Validation Checklist
Before making changes, verify:
1. Similar functionality doesn't already exist
2. Changes align with existing code patterns
3. Critical modifications are properly documented
4. No unnecessary defaults or fallbacks are introduced