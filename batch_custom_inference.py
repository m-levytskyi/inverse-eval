...
for exp_id, custom_priors in zip(experiments, custom_priors_list):
    ...
    print(
        f"Predicted SLD profile for {exp_id}: slds={slds_pred}, thicknesses={thicknesses_pred}, roughnesses={roughnesses_pred}"
    )

    # Ask user for parameters from other model
    print(
        f"Enter parameters from other model for {exp_id} (comma-separated, order: {', '.join(param_names)}):"
    )
    ...
    if other_params is not None and len(other_params) == 5:
        ...
        sld_prof_other = sld_profile(
            z_other, slds_other, interfaces_other, roughnesses_other
        )
        print(
            f"User-input SLD profile for {exp_id}: slds={slds_other}, thicknesses={thicknesses_other}, roughnesses={roughnesses_other}"
        )
    ...
    if model_file:
        try:
            ...
            for line in lines:
                ...
                if len(parts) >= 4:
                    ...
                    try:
                        ...
                        print(
                            f"Warning: Invalid SLD value '{parts[1]}' in {exp_id}, setting to 0."
                        )
                        sld = 0.0
                    try:
                        thickness = (
                            float(parts[2])
                            if parts[2] not in ["inf", "none", "NaN"]
                            else 0.0
                        )
                    except Exception:
                        print(
                            f"Warning: Invalid thickness value '{parts[2]}' in {exp_id}, setting to 0."
                        )
                        thickness = 0.0
                    try:
                        roughness = (
                            float(parts[3])
                            if parts[3] not in ["none", "inf", "NaN"]
                            else 3.0
                        )
                    except Exception:
                        print(
                            f"Warning: Invalid roughness value '{parts[3]}' in {exp_id}, setting to 3.0."
                        )
                        roughness = 3.0
                    layers.append(
                        {
            ...
            if len(layers) == 3:
                ...
                t_val_str = (
                    str(t_val_raw).strip().lower() if t_val_raw is not None else ""
                )
                try:
                ...
                if t_val is None or t_val_str in ["none", "inf", "nan"] or t_val <= 0:
                    print(
                        f"Warning: Film layer thickness is invalid ('{t_val_raw}') in {exp_id}, skipping ground truth SLD plot."
                    )
                    sld_prof_gt = None
                else:
                    ...
                    gt_amb_rough = (
                        layers[1]["roughness"]
                        if layers[1]["roughness"] is not None
                        else 3.0
                    )
                    gt_sub_rough = (
                        layers[2]["roughness"]
                        if layers[2]["roughness"] is not None
                        else 3.0
                    )
                    print(
                        f"  - Ground truth ambient/L1 roughness: {gt_amb_rough:.4f} Å (from layer '{layers[1]['name']}')"
                    )
                    print(
                        f"  - Ground truth L1/substrate roughness: {gt_sub_rough:.4f} Å (from layer '{layers[2]['name']}')"
                    )
                    roughnesses_gt = [gt_amb_rough, gt_sub_rough]
                    print(
                        f"  - Roughness values passed to sld_profile (ground truth): {roughnesses_gt}"
                    )
                    z_gt = np.linspace(0, t_gt + 50, 400)
                    interfaces_gt = [0] + list(np.cumsum(thicknesses_gt))
                    sld_prof_gt = sld_profile(
                        z_gt, slds_gt, interfaces_gt, roughnesses_gt
                    )
                    print(
                        f"Ground truth SLD profile for {exp_id}: slds={slds_gt}, thicknesses={thicknesses_gt}, roughnesses={roughnesses_gt}"
                    )
            elif len(layers) == 4:
                ...
                t1_val_str = (
                    str(t1_val_raw).strip().lower() if t1_val_raw is not None else ""
                )
                t2_val_str = (
                    str(t2_val_raw).strip().lower() if t2_val_raw is not None else ""
                )
                try:
                    t1 = float(t1_val_raw)
                except Exception:
                ...
                if (
                    t1 is None
                    or t1_val_str in ["none", "inf", "nan"]
                    or t1 <= 0
                    or t2 is None
                    or t2_val_str in ["none", "inf", "nan"]
                    or t2 <= 0
                ):
                    print(
                        f"Warning: One or both film layer thicknesses are invalid ('{t1_val_raw}', '{t2_val_raw}') in {exp_id}, skipping ground truth SLD plot."
                    )
                    sld_prof_gt = None
                else:
                    print(
                        f"Debug: Using film layer thicknesses {t1}, {t2} for {exp_id}"
                    )
                    t_gt = t1 + t2
                    weighted_sld = (
                        (layers[1]["sld"] * t1 + layers[2]["sld"] * t2) / t_gt
                        if t_gt > 0
                        else 0.0
                    )
    ...
    if model_file and sld_prof_gt is not None:
        ...
        if len(layers) == 3:
            ...
            t_gt = t1f + t2f
            weighted_sld = (
                (layers[1]["sld"] * t1f + layers[2]["sld"] * t2f) / t_gt
                if t_gt > 0
                else 0.0
            )
            table_rows.append(
