...
def main():
    ...
    prior_bounds = [
        (0.0, 100.0),
        (0.0, 700.0),  # layer thicknesses (SiO2, Polymer)
        (0.0, 5.0),
        (0.0, 50.0),
        (0.0, 80.0),  # roughnesses (Si/SiO2, SiO2/Polymer, Polymer/D2O)
        (3.47, 3.47),
        (-0.5, 3.0),
        (6.35, 6.35),  # SLDs (SiO2 fixed, Polymer, D2O fixed)
    ]
    ...
    q_exp, curve_exp, sigmas_exp = load_experimental_data(
        data_file, enable_preprocessing=False
    )
    ...
    for i, param_name in enumerate(param_names):
        ...
        if ref_val is not None:
            print(
                f"{param_name.ljust(18)} | {pred_val:9.2e} | {polished_val:9.2e} | {ref_val:9.2e}"
            )
        else:
            print(
                f"{param_name.ljust(18)} | {pred_val:9.2e} | {polished_val:9.2e} | {'(fixed)':>9}"
            )
