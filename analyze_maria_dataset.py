...
def count_active_layers(layers):
    ...
    active_layers = [
        layer
        for layer in layers
        if layer["name"] not in ["fronting", "backing"] and layer["thickness"] != np.inf
    ]
...
def analyze_dataset(dataset_path):
    ...
    # Get all model files
    model_files = sorted(
        [f for f in os.listdir(dataset_path) if f.endswith("_model.txt")]
    )
    ...
    for i, model_file in enumerate(model_files):
        ...
        try:
            ...
            curve_file = os.path.join(
                dataset_path, f"{experiment_id}_experimental_curve.dat"
            )
...
def create_visualizations(results, layer_details, q_ranges):
    ...
    for layer_pos, thicknesses in results["thickness_statistics"].items():
        for thickness in thicknesses:
            thickness_data.append(
                {"Layer Position": layer_pos, "Thickness (Å)": thickness}
            )

    if thickness_data:
        thickness_df = pd.DataFrame(thickness_data)
        sns.boxplot(
            data=thickness_df, x="Layer Position", y="Thickness (Å)", ax=axes[1, 0]
        )
    ...
    for layer_pos, roughnesses in results["roughness_statistics"].items():
        for roughness in roughnesses:
            roughness_data.append(
                {"Layer Position": layer_pos, "Roughness (Å)": roughness}
            )

    if roughness_data:
        roughness_df = pd.DataFrame(roughness_data)
        sns.boxplot(
            data=roughness_df, x="Layer Position", y="Roughness (Å)", ax=axes[1, 1]
        )
    ...
    plt.tight_layout()
    plt.savefig(
        "/home/levytskyi/Documents/reflectorch api playground/maria_dataset_analysis.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.show()
...
def print_summary_report(results, layer_details, q_ranges):
    ...
    for layer_pos, slds in results["sld_statistics"].items():
        if slds:
            print(
                f"     - {layer_pos}: {np.min(slds):.2e} to {np.max(slds):.2e} "
    ...
    for layer_pos, thicknesses in results["thickness_statistics"].items():
        if thicknesses:
            print(
                f"     - {layer_pos}: {np.min(thicknesses):.1f} to {np.max(thicknesses):.1f} "
    ...
    for layer_pos, roughnesses in results["roughness_statistics"].items():
        if roughnesses:
            print(
                f"     - {layer_pos}: {np.min(roughnesses):.1f} to {np.max(roughnesses):.1f} "
    ...
    if results["q_range_info"]:
        ...
        print(
            f"   • Q_min range: {qi['q_min_range'][0]:.4f} to {qi['q_min_range'][1]:.4f}"
        )
        print(
            f"   • Q_max range: {qi['q_max_range'][0]:.4f} to {qi['q_max_range'][1]:.4f}"
        )
        print(
            f"   • Number of Q points: {qi['n_points_range'][0]} to {qi['n_points_range'][1]} "
...
def save_analysis_results(results, layer_details, output_path):
    ...
    summary = {
        ...
        "layer_distribution": {
            str(k): int(v) for k, v in results["layer_distribution"].items()
        },
        "q_range_info": {
            ...
            layer_df.append(
                {
                    ...
                    "fronting_roughness": exp["fronting"]["roughness"]
                    if exp["fronting"]
                    else None,
                    "backing_sld": exp["backing"]["sld"] if exp["backing"] else None,
                    "backing_roughness": exp["backing"]["roughness"]
                    if exp["backing"]
                    else None,
                ...
                layer_df.append(
                    {
                        ...
                        "fronting_sld": exp["fronting"]["sld"]
                        if exp["fronting"]
                        else None,
                        "fronting_roughness": exp["fronting"]["roughness"]
                        if exp["fronting"]
                        else None,
                        "backing_sld": exp["backing"]["sld"]
                        if exp["backing"]
                        else None,
                        "backing_roughness": exp["backing"]["roughness"]
                        if exp["backing"]
    ...
    if layer_df:
        pd.DataFrame(layer_df).to_csv(
            f"{output_path}/maria_dataset_layers.csv", index=False
        )
...
def main():
    ...
    dataset_path = (
        "/home/levytskyi/Documents/reflectorch api playground/data/MARIA_VIPR_dataset"
    )
    ...
    if existing_df is not None:
        ...
        print(
            f"Layer-specific statistics saved to: {output_path}/maria_dataset_layer_statistics.json"
        )
