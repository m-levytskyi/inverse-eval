...
NF_CONFIG_NAME = (
    "nf_config_mixed_sigmas_qweighted.yaml"  # used when INFERENCE_BACKEND == "nf"
)
NF_NUM_SAMPLES = 1000
NF_DISABLE_IMPORTANCE_SAMPLING = False

# Sigmas input channel configuration
USE_SIGMAS_AS_INPUT = True  # Use sigmas as additional input channel to neural network (requires 2-channel model)
...
SWEEP_RESULTS_DIR = "sweep_results_nf_mixed_sigmas_qweighted"
...
class BatchPipelineSweep:
    ...
    def __init__(
        ...
        if self.num_experiments is None:
            ...
            print(
                f"📊 Discovered {self.num_experiments} experiments with {self.layer_count} layer(s)"
            )

        # Create timestamped sweep directory
        timestamp = datetime.now().strftime("%d%B%Y_%H_%M").lower()
        exp_desc = (
            f"ALL{self.num_experiments}"
            if self.num_experiments > 100
            else str(self.num_experiments)
        )
        self.sweep_dir = (
            Path(SWEEP_RESULTS_DIR)
            / f"sweep_{exp_desc}exps_{self.layer_count}layers_{timestamp}"
        )
        ...
        self.total_runs = (
            len(PRIOR_DEVIATIONS) * len(SLD_MODES) * len(PROMINENT_FEATURES)
        )
        self.current_run = 0
        ...
        if INFERENCE_BACKEND == "nf":
            ...
            print(
                f"NF importance sampling: {'disabled' if NF_DISABLE_IMPORTANCE_SAMPLING else 'enabled'}"
            )
    ...
    def run_single_configuration(self, prior_deviation, sld_mode, use_prominent):
        ...
        # Sigmas input channel flag
        if USE_SIGMAS_AS_INPUT:
        ...
        try:
            ...
            result = subprocess.run(
                cmd, capture_output=True, text=True, timeout=timeout_hours * 3600
            )
            ...
            if success:
                print(
                    f"✅ Configuration completed successfully in {execution_time:.1f} seconds"
                )
            else:
            ...
            timeout_hours = 6 if self.num_experiments > 100 else 1
            print(
                f"⏰ Configuration timed out after {execution_time:.1f} seconds ({timeout_hours} hour limit)"
            )
    ...
    def run_full_sweep(self):
        ...
        print(
            f"Estimated duration: {estimated_min} - {estimated_max} minutes ({estimated_min / 60:.1f} - {estimated_max / 60:.1f} hours)"
        )
        ...
        print(f"{'=' * 80}")
        print(
            f"Total time: {total_sweep_time:.1f} seconds ({total_sweep_time / 60:.1f} minutes)"
        )
