...
def generate_random_prediction(param_names: List[str], prior_bounds: List[Tuple[float, float]]) -> np.ndarray:
    """
    ...
    """
    random_params = []
    for i, (param_name, (min_val, max_val)) in enumerate(zip(param_names, prior_bounds)):
...
def calculate_mape_for_prediction(pred_params: np.ndarray, true_params: np.ndarray, 
                                  param_names: List[str], priors_type: str = None) -> float:
    """
    ...
    if np.any(nonzero_mask):
        percentage_errors[nonzero_mask] = np.abs(errors[nonzero_mask] / true_params[nonzero_mask]) * 100
    if np.any(zero_mask):
    ...
    if priors_type == "constraint_based":
        ...
        for i in range(len(errors)):
            ...
            try:
                ...
                print(f"Warning: Could not find constraint width for {param_name} (standardized: {standardized_name})")
...
def evaluate_batch_against_random(batch_dir: Path) -> Tuple[List[float], List[float]]:
    ...
    successful_results = {k: v for k, v in batch_results.items() 
    ...
    for exp_id, result in successful_results.items():
        ...
        random_mape = calculate_mape_for_prediction(random_pred, true_params, 
                                                   param_names, priors_type)
        if random_mape >= 0:
...
def plot_comparison_histogram(model_mapes: List[float], random_mapes: List[float],
                              batch_name: str, output_path: Path, priors_type: str = None,
                              layer_count: int = 1, narrow_priors_deviation: float = 0.99,
                              use_prominent_features: bool = False, fix_sld_mode: str = "none",
                              failed_count: int = 0, outlier_count: int = 0):
    """
    Create comparison histogram with model and random-guess MAPEs using original layout.
    ...
    mape_type_label = "Constraint-Based MAPE" if priors_type == "constraint_based" else "MAPE"
    ...
    fig.suptitle(f'{mape_type_label} Distribution - {len(model_mapes)} {layer_count}-Layer Experiments{title_suffix}\n'
    ...
    for i in range(len(mape_ranges) - 1):
        count = sum(1 for mape in model_mapes if mape_ranges[i] <= mape < mape_ranges[i+1])
        model_counts.append(count)
    
    # Count random MAPEs in each range
    random_counts = []
    for i in range(len(mape_ranges) - 1):
        count = sum(1 for mape in random_mapes if mape_ranges[i] <= mape < mape_ranges[i+1])
        random_counts.append(count)
    
    # Create bar chart (matching original colors)
    colors = plt.cm.RdYlGn_r(np.linspace(0.2, 0.8, len(model_counts)))
    bars = ax.bar(range(len(model_counts)), model_counts, color=colors, alpha=0.8, edgecolor='black')
    
    # Overlay random-guess histogram as semi-transparent grey
    ax.bar(range(len(random_counts)), random_counts, color='grey', alpha=0.3, edgecolor='darkgrey', 
           linewidth=1.5)
...
def process_batches(batch_numbers: List[int]):
    ...
    for batch_num in batch_numbers:
        ...
        if len(batch_dirs) > 1:
            print(f"Warning: Multiple directories found for batch {batch_num}, using first")
        ...
        try:
            ...
            outlier_count = sum(1 for v in batch_results.values() 
                              if v.get('excluded_as_outlier', False))
            failed_count = sum(1 for v in batch_results.values() 
            ...
            plot_comparison_histogram(
                model_mapes, random_mapes, 
                batch_dir.name, output_path, priors_type,
                layer_count, narrow_priors_deviation,
                use_prominent_features, fix_sld_mode,
...
def generate_synthetic_random_evaluation(num_experiments: int = 3000, 
                                        layer_count: int = 1,
    ...
    for param_name in param_names:
        ...
        print(f"  {param_name:20s} -> {standardized:15s}: [{min_val:7.2f}, {max_val:7.2f}] (width: {width:.2f})")
    
    print()
    print("Generating random true values and predictions...")
    print("Using get_constraint_based_prior_bounds() to ensure physical constraints are respected")
    print()
    ...
    for i in range(num_experiments):
        ...
        mape = calculate_mape_for_prediction(pred_params, true_params, param_names, "constraint_based")
    ...
    fig_params.suptitle(f'Per-Parameter MAPE Distributions - Synthetic Random Evaluation\n'
    ...
    if output_path:
        per_param_path = output_path.parent / f"{output_path.stem}_per_parameter{output_path.suffix}"
    ...
    print("="*80)
    print(f"{'Parameter':<20} {'Mean':<10} {'Median':<10} {'Std Dev':<10} {'Theoretical':<12}")
    print("="*80)
    for param_name in param_names:
        param_mapes = per_param_mapes[param_name]
        print(f"{param_name:<20} {np.mean(param_mapes):>8.2f}%  {np.median(param_mapes):>8.2f}%  "
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "--synthetic":
        print("Running synthetic random evaluation (both true and predicted are random)")
        from pathlib import Path
        ...
        print(f"Evaluating batches {batch_numbers[0]} to {batch_numbers[-1]} against random guessing")
        print(f"Generating ONE random prediction per experiment\n")
