...
class ReflectometryTest:
    ...
    def __init__(self, experiment_id="s005888", tolerance=0.10, 
                 fit_quality_threshold=0.99):
    ...
    def validate_parameters(self, prediction_dict):
        ...
        for expected_name, expected_value in self.expected_params.items():
            ...
            relative_error = abs(predicted_value - expected_value) / abs(expected_value) if expected_value != 0 else abs(predicted_value)
            
            # Check if within tolerance
            within_tolerance = relative_error <= self.tolerance
            status = "PASS" if within_tolerance else "FAIL"
            
            print(f"{expected_name:15} | Pred: {predicted_value:8.2f} | "
        ...
        if failed_validations:
            error_msg = f"Parameter validation failed for: {', '.join(failed_validations)}"
    ...
    def run_test(self):
        ...
        try:
            ...
            if not data_file:
                raise FileNotFoundError(f"Could not find data file for {self.experiment_id}")
            ...
            # 4. Run inference
            print("\n4. Running inference...")
            q_model, prediction_dict = self.run_inference(q_exp, curve_exp, prior_bounds)
