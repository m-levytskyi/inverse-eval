...
def main():
    ...
    prior_bounds = [
        (1.0, 400.0),
        (1.0, 10.0),  # layer thicknesses (top to bottom)
        (0.0, 30.0),
        (0.0, 30.0),
        (0.0, 30.0),  # interlayer roughnesses (top to bottom)
        (10.0, 13.0),
        (20.0, 21.0),
        (20.0, 21.0),  # real layer slds (top to bottom)
    ]
    ...
    q_exp, curve_exp, sigmas_exp = load_experimental_data(
        data_file, enable_preprocessing=True
    )
    ...
    for i, param_name in enumerate(param_names):
        ...
        for ref_name, ref_value in reference_values.items():
            if (
                param_name in ref_name
                or ref_name.lower().replace(" ", "_") in param_name.lower()
            ):
                ref_val = ref_value
                break
        ...
        if ref_val is not None:
            print(
                f"{param_name.ljust(18)} | {pred_val:9.2f} | {polished_val:9.2f} | {ref_val:9.2f}"
            )
        else:
            print(
                f"{param_name.ljust(18)} | {pred_val:9.2f} | {polished_val:9.2f} | {'N/A':>9}"
            )
