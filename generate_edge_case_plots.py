...
def extract_mape_scores(all_results):
    ...
    for exp_id, exp_result in all_results.items():
        for priors_type in ["broad", "narrow"]:
            ...
            exp_mapes = []
            for model_name, model_result in priors_result.get(
                "models_results", {}
            ).items():
                if (
                    model_result.get("success", False)
                    and "parameter_metrics" in model_result
                ):
...
def plot_sld_comparison(exp_id, model_result, priors_type, mape, ax):
    ...
    if (
        "sld_profile_x" not in model_result
        or "sld_profile_predicted" not in model_result
        or not model_result["sld_profile_x"]
        or not model_result["sld_profile_predicted"]
    ):
        ax.text(
            0.5,
            0.5,
    ...
    if (
        "sld_profile_polished" in model_result
        and model_result["sld_profile_polished"]
        and len(model_result["sld_profile_polished"]) > 0
    ):
        try:
    ...
    if (
        len(actual_sld) > 0
        and len(actual_sld) == len(predicted_sld)
        and not np.array_equal(actual_sld, predicted_sld)
    ):
...
def main():
    parser = argparse.ArgumentParser(
        description="Generate edge case and good prediction plots"
    )
    parser.add_argument(
        "--json-file", type=str, required=True, help="Path to batch summary JSON file"
    )
    parser.add_argument(
        "--output-dir",
        type=str,
    ...
    for priors_type in ["broad", "narrow"]:
        ...
        sorted_cases = sorted(
            mape_scores[priors_type].items(), key=lambda x: x[1], reverse=True
        )
    ...
    for priors_type in ["broad", "narrow"]:
        ...
        sorted_cases = sorted(mape_scores[priors_type].items(), key=lambda x: x[1])
