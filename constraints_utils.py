...
def get_constraint_range(param_name: str) -> Tuple[float, float]:
    ...
    if param_name not in ranges:
        raise KeyError(f"Unknown parameter: {param_name}. Available: {list(ranges.keys())}")
...
def get_constraint_width(param_name: str) -> float:
    ...
    if param_name not in widths:
        raise KeyError(f"Unknown parameter: {param_name}. Available: {list(widths.keys())}")
...
def get_parameter_info(param_name: str) -> Dict:
    ...
    if param_name not in constraints['constraints']:
        raise KeyError(f"Unknown parameter: {param_name}. Available: {list(constraints['constraints'].keys())}")
