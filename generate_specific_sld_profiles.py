        ...
        for priors_type in priors_types:
            ...
            try:
                ...
                for model_name, model_result in pipeline.results.items():
                    if model_result["success"]:
                        ...
                        models_results[model_name] = {
                            ...
                            "sld_profile_x": model_result.get(
                                "sld_profile_x", []
                            ).tolist()
                            if hasattr(model_result.get("sld_profile_x", []), "tolist")
                            else model_result.get("sld_profile_x", []),
                            "sld_profile_predicted": model_result.get(
                                "sld_profile_predicted", []
                            ).tolist()
                            if hasattr(
                                model_result.get("sld_profile_predicted", []), "tolist"
                            )
                            else model_result.get("sld_profile_predicted", []),
                            "sld_profile_polished": model_result.get(
                                "sld_profile_polished", []
                            ).tolist()
                            if hasattr(
                                model_result.get("sld_profile_polished", []), "tolist"
                            )
                            else model_result.get("sld_profile_polished", []),
                        }

                        # Track best model (lowest overall MAPE)
                        if (
                            model_result["parameter_metrics"]
                            and "overall" in model_result["parameter_metrics"]
                        ):
                            overall_mape = model_result["parameter_metrics"]["overall"][
                                "mape"
                            ]
                ...
                exp_results["priors"][priors_type] = {
                    "success": len(
                        [r for r in models_results.values() if r.get("success", False)]
                    )
                    > 0,
...
def main():
    ...
    edge_results = run_specific_experiments(
        edge_cases, layer_count=1, priors_types=["broad"]
    )

    # Run inference on good cases
    print("\nProcessing good cases...")
    good_results = run_specific_experiments(
        good_cases, layer_count=1, priors_types=["narrow"]
    )
