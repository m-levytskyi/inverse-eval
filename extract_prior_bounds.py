    ...
    for param_type in ['sld', 'thickness', 'roughness']:
        if param_type in param_stats:
            ...
            for layer_pos, stats in param_stats[param_type].items():
                ...
                if param_type == 'sld':
                    print(f"  {layer_pos}: [{stats['min']:.2e}, {stats['max']:.2e}] (mean: {stats['mean']:.2e})")
                else:
                    print(f"  {layer_pos}: [{stats['min']:.1f}, {stats['max']:.1f}] (mean: {stats['mean']:.1f})")
...
def generate_reflectorch_prior_bounds(layer_count=2):
    ...
    if 'by_position' in data['parameters']:
        ...
        for i in range(1, layer_count + 1):
            ...
            if layer_key_pos in data['parameters']['by_position']:
                thickness = data['parameters']['by_position'][layer_key_pos]['thickness']
                bounds_list.append((thickness['min'], thickness['max']))
                param_names.append(f'thickness_layer_{i}')
                print(f"  Layer {i}: ({thickness['min']:.1f}, {thickness['max']:.1f}) - mean: {thickness['mean']:.1f}")
    ...
    if 'interface' in data['parameters']:
        ...
        print(f"  Fronting: ({front_rough['min']:.1f}, {front_rough['max']:.1f}) - mean: {front_rough['mean']:.1f}")
        ...
        if 'by_position' in data['parameters']:
            for i in range(1, layer_count + 1):
                layer_key_pos = f'layer_{i}'
                if layer_key_pos in data['parameters']['by_position']:
                    roughness = data['parameters']['by_position'][layer_key_pos]['roughness']
                    bounds_list.append((roughness['min'], roughness['max']))
                    param_names.append(f'roughness_layer_{i}')
                    print(f"  Layer {i}: ({roughness['min']:.1f}, {roughness['max']:.1f}) - mean: {roughness['mean']:.1f}")
    ...
    if 'by_position' in data['parameters']:
        for i in range(1, layer_count + 1):
            ...
            if layer_key_pos in data['parameters']['by_position']:
                ...
                print(f"  Layer {i}: ({sld['min']:.2e}, {sld['max']:.2e}) - mean: {sld['mean']:.2e}")
    ...
    if 'interface' in data['parameters']:
        ...
        print(f"  Substrate: ({back_sld['min']:.2e}, {back_sld['max']:.2e}) - mean: {back_sld['mean']:.2e}")
