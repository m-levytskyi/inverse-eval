...
def compare_sld_profiles(
    experiment_id,
    data_dir="./data/MARIA_VIPR_dataset/2",
    output_dir="./sld_plots/2",
    save=True,
    show=True,
):
    """
    ...
    try:
        ...
        z_calc, sld_calc, true_params_dict = calculate_sld_from_model(
            model_file, z_range=(z_dat.min(), z_dat.max()), n_points=len(z_dat)
        )
        ...
        # Plot both profiles
        plot_sld_profile(z_dat, sld_dat, label="From sld_profile.dat", ax=ax)
        plot_sld_profile(z_calc, sld_calc, label="Calculated from model.txt", ax=ax)

        # Customize plot
        ax.set_title(
            f"SLD Profile Comparison - {experiment_id}", fontsize=14, fontweight="bold"
        )
...
def batch_process_directory(
    data_dir="./data/MARIA_VIPR_dataset/1",
    output_dir="./sld_plots/1",
    show=False,
...
def batch_compare_sld_profiles(
    experiment_ids,
    data_dir="./data/MARIA_VIPR_dataset/1",
    output_dir="./sld_plots/1",
    save=True,
...
def main():
    ...
    parser.add_argument(
        "experiment_ids",
        nargs="*",
        help="Experiment identifier(s) (e.g., s000000). If none provided, processes entire directory",
    )
    parser.add_argument(
        "--data-dir",
        default="./data/MARIA_VIPR_dataset/2",
        help="Directory containing data files",
    )
    parser.add_argument(
        "--output-dir", default="./sld_plots/2", help="Directory to save plots"
    )
    ...
    parser.add_argument(
        "--batch",
        action="store_true",
        help="Process multiple experiments (implied if multiple IDs given)",
    )
    parser.add_argument(
        "--max-experiments",
        type=int,
        help="Maximum number of experiments to process (for directory batch processing)",
    )
